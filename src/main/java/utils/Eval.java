...
public class Eval {
    ...
    public Vector reduceTokens(Vector tokens) {
        ...
        while (tokens.indexOf("(") != -1) {
            ...
            for (int it = ib + 1; it < tokens.size(); it++) {
                ...
                if (brackets == 0) {
                    ...
                    //Logging.log("After reduce tokens Diff "+(startsize-
                    // endsize));
                    // for (int i = 0; i < bracketTokens.size(); i++)
                    // Logging.log(" " +
                    // (String)bracketTokens.elementAt(i));
                    ...
                    // Logging.log("Replaced tokens ");
                    // for (int i = 0; i < tokens.size(); i++)
                    // Logging.log(" " + (String)tokens.elementAt(i));
                    break;
                }
            }
        ...
        while (tokens.size() > 1) {
            ...
            String st = (String) tokens.elementAt(it);
            int iop = getOperator(st);
            // Logging.log("     Precedence Operator: "+it+" " + st);
